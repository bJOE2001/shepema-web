...
def remove_background_pure_pil(input_path, output_path):
    img = Image.open(input_path).convert("RGBA")
    w, h = img.size
    pixels = img.load()
    
    # Sample corner pixels for average background color
    samples = [
        pixels[0, 0], pixels[w-1, 0], pixels[0, h-1], pixels[w-1, h-1],
        pixels[5, 5], pixels[w-6, 5], pixels[5, h-6], pixels[w-6, h-6]
    ]
    ...
    print(f"BG RGB: {bg_r:.1f}, {bg_g:.1f}, {bg_b:.1f}")
...
remove_background_pure_pil(
    r"c:\xampp\htdocs\shepema-web\public\images\mascot-cozy-reading.jpg",
    r"c:\xampp\htdocs\shepema-web\public\images\mascot-cozy-reading-transparent.png"
)
